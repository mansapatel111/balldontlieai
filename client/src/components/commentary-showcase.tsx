...
const DEMO_COMMENTS = [
  ...
  {
    id: 6,
    user: "@transcript_live",
    text: "[INAUDIBLE SCREAMING] followed by the sound of ankles shattering in 4K resolution.",
    likes: "33K",
    vibe: "Literal"
  },
  {
      ...
      <div className="relative w-full h-[500px]">
        {DEMO_COMMENTS.map((comment, idx) => (
          <motion.div
            ...
            style={{
              position: 'absolute',
              top: `${10 + Math.floor(idx / 3) * 35}%`,
              left: `${5 + (idx % 3) * 32}%`,
              zIndex: Math.floor(Math.random() * 10)
            }}
            className="w-full max-w-[320px] bg-black/40 border border-white/10 rounded-xl p-5 backdrop-blur-md shadow-xl hover:bg-white/10 hover:scale-105 hover:z-50 transition-all duration-300 cursor-pointer"
          >
