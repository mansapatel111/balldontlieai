...
export function CyberneticGridShader() {
  ...
  useEffect(() => {
    ...
    // Updated fragment shader to use purple/pink like the emojis
    const fragmentShader = `
      ...
      void main() {
        ...
        // base grid color pulsing (Updated to Purple/Indigo)
        vec3 gridColor = vec3(0.6, 0.1, 1.0); // Purple base
        vec3 color     = gridColor
                       * line
                       * (0.5 + sin(t * 2.0) * 0.2);

        // energetic pulses along grid (Updated to Pink/Magenta)
        float energy = sin(uv.x * 20.0 + t * 5.0)
                     * sin(uv.y * 20.0 + t * 3.0);
        energy = smoothstep(0.8, 1.0, energy);
        color += vec3(1.0, 0.2, 0.8) * energy * line; // Pink energy

        // glow around mouse
        float glow = smoothstep(0.1, 0.0, mouseDist);
        color += vec3(0.8, 0.4, 1.0) * glow * 0.5; // Light purple glow
